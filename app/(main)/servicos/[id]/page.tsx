...
export default function ServiceDetailPage() {
  ...
  type Service = {
    ...
    photos?: Photo[];
    bids?: Bid[];
    completionConfirmations?: { userId: string }[]; // Added this to type for clarity
  };

  const [service, setService] = useState<Service | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState('');
  const [bidForm, setBidForm] = useState({ price: '', message: '', proposedDate: '' });
  ...
  // This 'acceptedBid' is defined early for use in promptReview useEffect dependency.
  // It will be re-calculated later AFTER service is confirmed loaded for other logic.
  const acceptedBidForEffect = useMemo(() => service?.bids?.find(bid => bid.status === 'ACCEPTED'), [service]);

  useEffect(() => {
    if (!service || !session?.user || !acceptedBidForEffect || !searchParams) return;
    ...
    if (shouldPromptReview && localIsCreatorReviewPending && acceptedBidForEffect.provider) {
      setServiceProviderForReview({
        id: acceptedBidForEffect.provider.id,
        name: acceptedBidForEffect.provider.name || 'Prestador Desconhecido',
        image: acceptedBidForEffect.provider.image,
      });
      setShowReviewPopup(true);
      const newPath = window.location.pathname;
      router.replace(newPath, undefined);
    }
  }, [service, session, router, showReviewPopup, acceptedBidForEffect, searchParams]);
  ...
  const callApi = async (url: string, method: string, body?: any, successMessage?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        (errorToThrow as any).apiData = data;
        throw errorToThrow;
      }
  ...
  const handleConfirmCompletion = async () => {
    ...
    if (result && result.serviceStatus === 'COMPLETED') {
      const currentIsCreator = session?.user?.id === currentServiceState.creatorId; // Use currentServiceState
      if (currentIsCreator) {
        const currentAcceptedBid = currentServiceState.bids?.find(bid => bid.status === 'ACCEPTED');
        if (currentAcceptedBid && currentAcceptedBid.provider) {
          setServiceProviderForReview({
            id: currentAcceptedBid.provider.id,
            name: currentAcceptedBid.provider.name || 'Prestador Desconhecido',
            image: currentAcceptedBid.provider.image,
          });
          setShowReviewPopup(true);
        }
      }
  ...
  const UserProfileModal = ({ userId: modalUserId, onClose }: { userId: string, onClose: () => void }) => {
    ...
    useEffect(() => {
      const fetchUserProfile = async () => {
        try {
          const response = await fetch(`/api/users/${modalUserId}/profile`);
    ...
    if (profileLoading) return <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="bg-white rounded-lg p-6 max-w-md w-full"><p className="text-center">Carregando perfil...</p></div></div>;
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full">
          <div className="flex justify-between items-start mb-4"><h2 className="text-xl font-bold">Perfil do Usuário</h2><button onClick={onClose} className="text-gray-500 hover:text-gray-700"><XMarkIcon /></button></div>
          {profileUser ? (
            <div>
              <div className="flex items-center space-x-4 mb-4">
                <div className="h-16 w-16 rounded-full overflow-hidden bg-gray-200">{profileUser.image ? <img src={profileUser.image} alt={profileUser.name} className="h-full w-full object-cover" /> : <div className="h-full w-full flex items-center justify-center bg-primary-100 text-primary-600 text-xl font-bold">{profileUser.name?.charAt(0) || '?'}</div>}</div>
                <div><h3 className="text-lg font-semibold">{profileUser.name}</h3><div className="flex items-center"><span className="text-yellow-400">★</span><span className="ml-1 text-sm text-gray-600">{profileUser.rating?.toFixed(1) || 'Sem avaliações'}</span></div></div>
              </div>
              {profileUser.about && (<div className="mb-4"><h4 className="text-sm font-medium text-gray-700 mb-1">Sobre</h4><p className="text-sm text-gray-600">{profileUser.about}</p></div>)}
              {profileUser.professions && profileUser.professions.length > 0 && (<div className="mb-4"><h4 className="text-sm font-medium text-gray-700 mb-1">Profissões</h4><div className="flex flex-wrap gap-2">{profileUser.professions.map((profession: any) => (<span key={profession.id} className="px-2 py-1 bg-primary-100 text-primary-800 text-xs rounded-full">{profession.name}</span>))}</div></div>)}
              {profileUser.reviews && profileUser.reviews.length > 0 && (<div><h4 className="text-sm font-medium text-gray-700 mb-1">Avaliações Recentes</h4><div className="space-y-2 max-h-40 overflow-y-auto">{profileUser.reviews.map((review: any) => (<div key={review.id} className="border-b border-gray-100 pb-2"><div className="flex items-center"><div className="text-yellow-400 mr-1">{'★'.repeat(review.rating)}</div><div className="text-gray-400 text-xs">{'★'.repeat(5 - review.rating)}</div></div><p className="text-sm text-gray-600">{review.comment}</p><p className="text-xs text-gray-500">{new Date(review.createdAt).toLocaleDateString()}</p></div>))}</div></div>)}
            </div>
          ) : (<p className="text-center text-gray-600">Não foi possível carregar o perfil.</p>)}
          <div className="mt-4 space-y-2"><Link href={`/perfil/${modalUserId}`} passHref><button className="w-full px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md text-sm font-medium text-center">Ver Perfil Completo</button></Link><button onClick={onClose} className="w-full px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-md text-sm font-medium">Fechar</button></div>
        </div>
      </div>
    );
  };
  ...
  //isLoading, error, and service are state variables managed by hooks (useState, useCallback->useEffect).

  if (isLoading) return <div className="flex justify-center items-center min-h-screen"><p>Carregando detalhes do serviço...</p></div>;
  if (error && !service) return <div className="max-w-4xl mx-auto px-4 py-12 text-center"><p className="text-red-600">{error}</p><Link href="/explorar" className="mt-4 inline-block btn-primary">Voltar</Link></div>;
  if (!service) return <div className="max-w-4xl mx-auto px-4 py-12 text-center"><p className="text-red-600">Serviço não encontrado.</p><Link href="/explorar" className="mt-4 inline-block btn-primary">Voltar</Link></div>;
  ...
  const {
    ...
    // service, session, acceptedBid are guaranteed to be available here due to early returns.
    // However, session.user might still be null if not logged in, so keep optional chaining for session.user.id
    if (service && acceptedBid) { // session.user is checked by isCreator/canCurrentUserBid etc.
      const creatorId = service.creatorId;
      const providerId = acceptedBid.providerId;
      const currentUserId = session?.user?.id;

      if (currentUserId && service.completionConfirmations) {
      ...
      if (service.status === 'COMPLETED' && session?.user?.id === service.creatorId) {
        ICRP = !showReviewPopup;
      }

      console.log({
        currentUserId_memo: session?.user?.id,
  ...
  }, [service, session, acceptedBid, showReviewPopup]);
